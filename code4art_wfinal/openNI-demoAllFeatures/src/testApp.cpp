...
void testApp::setupRecording(string _filename) {
    ...
    // use this to set dynamic max number of users (NB: that a hard upper limit is defined by MAX_NUMBER_USERS in ofxUserGenerator)
    ...
    // custom smoothing/filtering (can also set per hand with setFilterFactor)...set them all to 0.1f to begin with
...
//	gui->addWidgetDown(new ofxUISlider(length-xInit,dim, 0.0, 20.0, boxmultiplier, "Boxsize Multiplier")); 
	...
	gui->addWidgetDown(new ofxUIToggle( dim, dim, false, "isRecording")); 
	gui->addWidgetDown(new ofxUIToggle( dim, dim, false, "isLive")); 
