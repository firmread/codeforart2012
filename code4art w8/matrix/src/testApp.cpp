...
void testApp::setup(){
    ...
    boatX = 1;
    for (int i = 0; i<3; i++) {
        snowmanScale[i] = ofRandom(.65,.9);
        snowmanPosition[i].x = ofRandom(20,300);
    }
}
...
void testApp::draw(){
    ...
    ofPushMatrix(); // save the prestine matrix
    ofTranslate(boatX, 420); // mess up with matrix
    //we are now in boat matrix
    
    for (int i = 0; i<3; i++) {
        ofPushMatrix(); // saving boat matrix
        ofTranslate(snowmanPosition[i].x, 70);
        ofScale(snowmanScale[i], snowmanScale[i]);
        
        drawSnowman();
        ofPopMatrix(); //restore boat matrix
    ...
    ofSetColor(10, 80, 205);
    for (int i=0; i < ofGetWidth(); i+=30) {
        ofCircle(i, ofGetHeight(), 90);
    }
}
